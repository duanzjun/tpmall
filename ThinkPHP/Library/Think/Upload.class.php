...
class Upload
{
    ...
    function __construct($options = null, $initialize = true, $error_messages = null) {
        ...
        $this->options = array(
            ...
            'readfile_chunk_size' => 10 * 1024 * 1024, // 10 MB
    ...
    protected function fix_file_extension($file_path, $name, $size, $type, $error,
        ...
        if (strpos($name, '.') === false &&
            ...
            //修正扩展名jpeg为jpg
            $name .= '.'.str_replace($matches[1],'jpeg','jpg');
        }
    ...
    protected function get_file_name($file_path, $name, $size, $type, $error, $index, $content_range) {
        if($this->options['rule'][0]=='custom' && !empty($this->options['rule'][1])){
            $name = $this->trim_file_name($file_path,$this->options['rule'][1], $size, $type, $error, $index, $content_range);
            return $this->fix_file_extension($file_path, $name, $size, $type, $error,
                $index, $content_range);
        }else{
            //生成文件名
            $func     = $this->options['rule'][0];
            $param    = (array)$this->options['rule'][1];
            foreach ($param as &$value) {
               $value = str_replace('__FILE__', $name, $value);
            }
