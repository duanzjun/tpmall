$(function(){
    ...
    $('span[ectype="inline_edit"]').click(function(){
        ...
        $('<input type="text">').css({border:'1px solid #ccc',width:'80%'})
            .attr({value:s_value})
            ...
            .keyup(function(event){
                if(event.keyCode==13)
                {
                    if(req)
                    {
                        if(!required($(this).prop('value'),s_value,$(this))) return;
                    }
                    if(type)
                    {
                        if(!check_type(type,$(this).prop('value'),s_value,$(this))) return;
                    }
                    if(max)
                    {
                        if(!check_max($(this).prop('value'),s_value,max,$(this))) return;
                    }
                    $(this).prev('span').show().text($(this).prop("value"));
                    $.getJSON('index.php?m=admin&c='+app+'&a=ajax_col',{id:s_id,col:s_name,val:$(this).prop('value')},function(data){
    ...
    $('img[ectype="inline_edit"]').click(function(){
        ...
        $.get('index.php?app='+app+'&act=ajax_col',{id:i_id,column:i_name,value:i_val},function(data){
        if(data === 'true')
            {
    ...
    $('span[ectype="order_by"]').click(function(){
        ...
        location.assign(location.pathname+'?'+params.join('&'));
    });
});
//检查提交内容的必须项
