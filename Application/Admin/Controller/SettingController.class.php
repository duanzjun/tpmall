...
class SettingController extends AdminController
{
    ...
    {
        $this->_config_file=APP_PATH.'Common/Conf/setting.conf.php';
    }

    //系统设置
    public function index()
    {
        $settings=load_config($this->_config_file);
        if(IS_POST){
            $settings=array_merge($settings,$_POST);
            $new_config=var_export($settings,true);
            file_put_contents($this->_config_file, "<?php\r\nreturn {$new_config};\r\n");
            $this->success('设置成功');
        }else{
            $time_zone=$this->_get_time_zone();
            $this->get_default();
            $settings=array_merge($this->get_default(),(array)$settings);
    ...
    //二级域名设置
    public function subdomain_setting()
    {
        $this->index();
    }
    ...
    public function ajaxupload()
    {
        parent::ajaxUpload(array(
            'upload_url'=>'data/system/',    //图片显示地址
            'upload_dir'=>'data/system/',    //图片存放路径
    ...
    public function get_default()
    {
        return array(
            ...
            'site_keywords' => 'tpmall mall',
            'site_status' => '1',
            'closed_reason' => 'default_closed_reason',
            'email_type' => 1,
            'email_port' => '25',
    ...
    //时区列表
    public function _get_time_zone()
    {
        return array(
